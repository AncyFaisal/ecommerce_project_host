...
from apps.sanjeri_models import Cart, Address, Order, OrderItem, Coupon, Wallet
from apps.sanjeri_models import ProductOffer, CategoryOffer, OfferApplication
from apps.utils.offer_utils import apply_offers_to_cart, get_best_offer_for_product
from django.utils import timezone
...
def checkout_view(request):
    ...
    if 'applied_coupon' in request.session:
        ...
        try:
            ...
            # Check validity
            is_valid, message = coupon.is_valid(request.user, price_after_offers)
            if is_valid:
                coupon_discount = coupon.calculate_discount(price_after_offers)
    ...
    price_after_coupon = price_after_offers - coupon_discount
    ...
    'total_discount': float(offer_discount + coupon_discount),
    ...
    # Collect unique offer names applied to cart items
    applied_offer_names = list({
        offer['offer_name']
        for offer in offer_info['applied_offers']
    ...
    context = {
        ...
        'offer_discount': float(offer_discount),
        'applied_offer_names': applied_offer_names,
        'coupon_discount': float(coupon_discount),
        'total_discount': float(offer_discount + coupon_discount),
        'shipping_charge': float(shipping_charge),
        'tax_amount': float(tax_amount),
