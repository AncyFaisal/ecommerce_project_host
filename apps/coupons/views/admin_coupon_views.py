...
from apps.sanjeri_models import Coupon
        ...
        try:
            with transaction.atomic():
                ...
                if discount_type == 'fixed':
                    if not min_order_amount or Decimal(min_order_amount) == 0:
                        raise ValidationError("A minimum order amount is required for fixed amount coupons.")
                    if Decimal(discount_value) >= Decimal(min_order_amount):
                        raise ValidationError(
                            f"Fixed discount (₹{discount_value}) must be less than "
                            f"the minimum order amount (₹{min_order_amount})."
                        )
...
def view_coupon(request, coupon_id):
    ...
    return render(request, 'admin/coupon/view.html', context)
...
def edit_coupon(request, coupon_id):
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                valid_from_dt = user_timezone.localize(valid_from_dt).astimezone(pytz.UTC)
                valid_to_dt   = user_timezone.localize(valid_to_dt).astimezone(pytz.UTC)
                ...
                coupon.single_use_per_user  = single_use_per_user
                coupon.save()
    ...
    # Convert stored UTC datetimes to IST for the form
    valid_from_local = coupon.valid_from.astimezone(user_timezone).strftime('%Y-%m-%dT%H:%M')
    valid_to_local   = coupon.valid_to.astimezone(user_timezone).strftime('%Y-%m-%dT%H:%M')

    context = {
        'coupon': coupon,
        'valid_from_local': valid_from_local,
        'valid_to_local': valid_to_local,
    }
    return render(request, 'admin/coupon/edit.html', context)
