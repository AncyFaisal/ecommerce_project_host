...
from apps.sanjeri_models import Product
...
def product_offer_create(request):
    ...
    if request.method == 'POST':
        ...
        # Fixed discount must be less than min purchase amount
        if float(discount_fixed or 0) > 0 and float(min_purchase_amount or 0) > 0:
            if float(discount_fixed) >= float(min_purchase_amount):
                messages.error(request, f"Fixed discount (₹{discount_fixed}) must be less than the minimum purchase amount (₹{min_purchase_amount}).")
                return redirect('product_offer_create')
        if float(discount_fixed or 0) > 0 and float(min_purchase_amount or 0) == 0:
            messages.error(request, "A minimum purchase amount is required when using a fixed discount.")
            return redirect('product_offer_create')
...
def product_offer_edit(request, offer_id):
    ...
    if request.method == 'POST':
        # Validation for product offer edit
        _disc_fixed = request.POST.get('discount_fixed', 0)
        _min_amt = request.POST.get('min_purchase_amount', 0)
        if float(_disc_fixed or 0) > 0 and float(_min_amt or 0) > 0:
            if float(_disc_fixed) >= float(_min_amt):
                messages.error(request, f"Fixed discount (₹{_disc_fixed}) must be less than the minimum purchase amount (₹{_min_amt}).")
                return redirect('product_offer_edit', offer_id=offer_id)
        if float(_disc_fixed or 0) > 0 and float(_min_amt or 0) == 0:
            messages.error(request, "A minimum purchase amount is required when using a fixed discount.")
            return redirect('product_offer_edit', offer_id=offer_id)
...
def category_offer_list(request):
    ...
    offers = CategoryOffer.objects.prefetch_related('categories').all().order_by('-created_at')
...
def category_offer_edit(request, offer_id):
    ...
    if request.method == 'POST':
        # Validation for category offer edit
        _disc_fixed = request.POST.get('discount_fixed', 0)
        _min_amt = request.POST.get('min_purchase_amount', 0)
        if float(_disc_fixed or 0) > 0 and float(_min_amt or 0) > 0:
            if float(_disc_fixed) >= float(_min_amt):
                messages.error(request, f"Fixed discount (₹{_disc_fixed}) must be less than the minimum purchase amount (₹{_min_amt}).")
                return redirect('category_offer_edit', offer_id=offer_id)
        if float(_disc_fixed or 0) > 0 and float(_min_amt or 0) == 0:
            messages.error(request, "A minimum purchase amount is required when using a fixed discount.")
            return redirect('category_offer_edit', offer_id=offer_id)
    ...
    context = {
        'offer': offer,
        'categories': categories,
        'selected_category_ids': list(offer.categories.values_list('id', flat=True)),
        'now': timezone.now(),
        'title': f'Edit {offer.name}'
    }
    return render(request, 'admin/offers/category_offer_form.html', context)
