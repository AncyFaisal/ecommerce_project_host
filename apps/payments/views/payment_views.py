...
import razorpay
import json
from apps.sanjeri_models import Order
...
def payment_retry(request, order_id):
    """Reset a failed/pending payment so it can be retried via initiate_payment."""
    try:
        order = get_object_or_404(Order, id=order_id, user=request.user)

        retryable_statuses = ['failed', 'pending', 'partially_paid']

        if order.payment_status in retryable_statuses and order.can_pay_online:
            # Only clear Razorpay IDs for failed payments so a fresh order is created
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'success': True, 'message': 'Ready for payment'})
        else:
            msg = f'Order cannot be retried. Status: {order.status}, Payment: {order.payment_status}'
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
