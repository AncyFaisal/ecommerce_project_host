...
def generate_and_send_otp(email: str, username: str, purpose: str, ttl_seconds: int = 120) -> int:
    ...
    ctx = {"username": username, "otp": otp, "ttl_minutes": ttl_seconds // 60}

    template_map = {
        "signup":          ("Verify Your Email - SANJERI",       "emails/otp_signup.html"),
        "email_change":    ("Verify Email Change - SANJERI",     "emails/otp_email_change.html"),
        "password_change": ("Verify Password Change - SANJERI",  "emails/otp_password_change.html"),
    }
    subject, template = template_map.get(
        purpose,
        ("Password Reset OTP - SANJERI", "emails/otp_forgot_password.html")
    )
...
def user_signup(request):
    ...
    if request.method == "POST":
        ...
        if not re.match(r'^[a-zA-Z][a-zA-Z0-9_]+$', username):
            messages.error(request, "Username must start with a letter and contain only letters, numbers, or underscores.")
            return redirect("user_signup")
        ...
        if not phone or not re.match(r'^[6-9]\d{9}$', phone):
            messages.error(request, "Enter a valid 10-digit Indian mobile number starting with 6, 7, 8, or 9.")
            return redirect("user_signup")
