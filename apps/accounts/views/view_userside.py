...
from apps.sanjeri_models import Product, ProductVariant,Cart,CartItem
...
from apps.sanjeri_models import Wishlist,WishlistItem
from apps.sanjeri_models import ProductOffer, CategoryOffer
...
def men_products(request):
    ...
    gender_filter = request.GET.get('gender', 'Male')
    ...
    if price_range:
        if price_range == 'under-1000':
            variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).filter(effective_price__lt=1000)
        elif price_range == '1000-2000':
            variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).filter(effective_price__range=(1000, 2000))
        elif price_range == '2000-3000':
            variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).filter(effective_price__range=(2000, 3000))
        elif price_range == '3000-5000':
            variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).filter(effective_price__range=(3000, 5000))
        elif price_range == 'above-5000':
            variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).filter(effective_price__gt=5000)
    ...
    if volume:
        try:
            variants = variants.filter(volume_ml=int(volume))
        except ValueError:
            pass
    ...
    if sort_by == 'best-selling':
        ...
        variants = variants.annotate(effective_price=Coalesce('discount_price', 'price')).order_by('effective_price')
...
def women_products(request):
    ...
    from apps.products.models import Category
    available_categories = Category.objects.filter(
        products__variants__gender='Female',
        products__is_active=True,
        products__is_deleted=False,
        is_active=True,
        is_deleted=False
    ...
    # Build wishlist_variant_ids for product card wishlist buttons
    wishlist_variant_ids = []
    if request.user.is_authenticated:
        try:
            wishlist_obj = Wishlist.objects.get(user=request.user)
            wishlist_variant_ids = list(WishlistItem.objects.filter(wishlist=wishlist_obj).values_list('variant_id', flat=True))
        except Wishlist.DoesNotExist:
            pass
...
def unisex_products(request):
    ...
    from apps.products.models import Category
    available_categories = Category.objects.filter(
        products__variants__gender='Unisex',
        products__is_active=True,
        products__is_deleted=False,
        is_active=True,
        is_deleted=False
    ...
    # Build wishlist_variant_ids for product card wishlist buttons
    wishlist_variant_ids_unisex = []
    if request.user.is_authenticated:
        try:
            wishlist_obj = Wishlist.objects.get(user=request.user)
            wishlist_variant_ids_unisex = list(WishlistItem.objects.filter(wishlist=wishlist_obj).values_list('variant_id', flat=True))
        except Wishlist.DoesNotExist:
            pass
    ...
    context = {
        ...
        'cart_item_count': cart_item_count,
        'wishlist_variant_ids': wishlist_variant_ids_unisex,
        'active_filters': {
            'price_range': price_range,
            'fragrance_type': fragrance_type,
            'occasion': occasion,
