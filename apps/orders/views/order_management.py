...
from apps.sanjeri_models import Order, OrderItem
from apps.sanjeri_models import Wallet, WalletTransaction
...
def cancel_order_item(request, item_id):
    """Cancel specific order item (full or partial quantity) - only refund if payment was made"""
    ...
    # Parse cancel quantity (default to full quantity)
    try:
        cancel_quantity = int(request.POST.get('cancel_quantity', order_item.quantity))
        cancel_quantity = max(1, min(cancel_quantity, order_item.quantity))
    ...
    refund_amount = order_item.unit_price * cancel_quantity
    ...
    cancelled = order_item.cancel_item(reason, cancel_quantity)
    ...
    if cancelled:
        if user_paid and refund_amount > 0:
            try:
                transaction = WalletTransaction.objects.create(
                    ...
                    status='COMPLETED',
                    reason=f"Refund for cancelled item: {order_item.product_name} x{cancel_quantity} (Order #{order.order_number})",
                    order=order
                )
                ...
                messages.success(request, f"{order_item.product_name} x{cancel_quantity} cancelled. ₹{refund_amount} refunded to your wallet.")

            except Exception as e:
                messages.error(request, f"Item cancelled but refund failed: {str(e)}")
                return redirect('order_detail', order_id=order.id)
        else:
            messages.success(request, f"{order_item.product_name} x{cancel_quantity} cancelled. No refund needed.")
        ...
        if not remaining_items.exists():
            ...
            order.payment_status = 'refunded' if user_paid else 'cancelled'
            order.save()
            messages.info(request, "All items cancelled. Order marked as cancelled.")
...
def request_item_return(request, item_id):
    ...
    if request.method == 'POST':
        ...
        try:
            return_quantity = int(request.POST.get('return_quantity', order_item.quantity))
            return_quantity = max(1, min(return_quantity, order_item.quantity))
        except (ValueError, TypeError):
            return_quantity = order_item.quantity

        order_item.return_requested_quantity = return_quantity
        order_item.save(update_fields=['return_requested_quantity'])

        success, message = order_item.request_item_return(reason)

        if success:
            messages.success(request, f"Return request submitted for {return_quantity} unit(s). Awaiting admin approval.")
        else:
