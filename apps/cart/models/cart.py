...
class CartItem(models.Model):
    ...
    def offer_price(self):
        """Get the best offer-discounted price for this item's variant"""
        from django.utils import timezone
        from apps.sanjeri_models import ProductOffer, CategoryOffer
        from decimal import Decimal

        base_price = self.variant.display_price
        best_discount = Decimal('0')
        now = timezone.now()

        try:
            for offer in ProductOffer.objects.filter(
                products=self.variant.product,
                is_active=True,
                valid_from__lte=now,
                valid_to__gte=now
            ):
                discount, _ = offer.calculate_discount(base_price)
        ...
        try:
            if self.variant.product.category:
                for offer in CategoryOffer.objects.filter(
                    categories=self.variant.product.category,
                    is_active=True,
                    valid_from__lte=now,
                    valid_to__gte=now
                ):
        ...
        return max(base_price - best_discount, Decimal('0'))
