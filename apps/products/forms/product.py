...
from apps.sanjeri_models import Product, ProductVariant, Category
...
class ProductForm(forms.ModelForm):
    class Meta:
        model = Product
        fields = [
            'name', 'slug', 'description', 'main_image',
            'brand', 'fragrance_type', 'occasion',
            'category', 'is_featured', 'is_best_selling', 'is_new_arrival', 'is_active'
        ]
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter product name', 'id': 'id_name'}),
            'slug': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. dior-sauvage-100ml'}),
            ...
            'fragrance_type': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. Eau de Parfum'}),
            'occasion': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. Casual, Formal'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Default is_active to True for new products (no instance)
...
class ProductVariantForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # SKU is auto-generated, make it optional and readonly for display
        self.fields['sku'].required = False
        self.fields['sku'].widget.attrs.update({
            # 'placeholder': 'Auto-generated from product SKU + volume + gender',
            'readonly': 'readonly',
    ...
    def clean(self):
        ...
        # discount_price is optional — only validate if provided
        if discount_price is not None and price is not None:
            if discount_price > price:
                raise forms.ValidationError("Discount price must be less than or equal to the regular price.")
