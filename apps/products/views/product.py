...
from apps.sanjeri_models import Product, ProductVariant, ProductImage, Category
from apps.sanjeri_forms import ProductForm, ProductVariantFormSet, ProductVariantForm  # Fixed import
...
def product_add(request):
    if request.method == "POST":
        ...
        if form.is_valid() and variant_formset.is_valid():
            try:
                ...
                import re
                base_sku = re.sub(r'[^A-Za-z0-9]', '-', product.name).upper()[:20].strip('-')
                sku = base_sku
                counter = 1
                ...
                for variant in variants:
                    ...
                    # Clear SKU so model auto-generates it
                    variant.sku = ''
                    variant.save()
    ...
    return render(request, 'product_add.html', {
        'form': form,
        'product_formset': variant_formset
    })
# views/product.py
            ...
            try:
                ...
                for instance in instances:
                    if not instance.pk:
                        ...
                        instance.sku = ''  # Let model auto-generate
...
def product_detail(request, product_id):
    ...
    try:
        ...
        variants = product.variants.filter(is_active=True, is_deleted=False, stock__gt=0)
        if not variants.exists():
            raise Http404("This product is currently unavailable or out of stock.")
        ...
        # Build gender-based breadcrumb
        first_variant = variants.first()
        if first_variant:
            gender = first_variant.gender
            if gender == 'Male':
                gender_name = "Men"
                gender_url = "/men/"
            elif gender == 'Female':
            ...
            gender_name = product.category.name
            gender_url = "#"

        breadcrumbs = [
            {'name': 'Home', 'url': '/'},
            {'name': gender_name, 'url': gender_url},
