...
class ProductVariant(models.Model):
    ...
    def get_best_offer(self):
        """Return the best active offer for this variant (product or category offer)"""
        from django.utils import timezone
        ...
        try:
            for offer in ProductOffer.objects.filter(
                ...
                discount, _ = offer.calculate_discount(self.display_price)
    ...
    def offer_price(self):
        """Return the final price after applying the best offer"""
        from decimal import Decimal
        offer = self.get_best_offer()
        if offer:
            discount, _ = offer.calculate_discount(self.display_price)
            return max(self.display_price - discount, Decimal('0'))
    ...
    def min_price(self):
        ...
        if variants.exists():
            return min(variant.display_price for variant in variants)
        return 0
    ...
    def max_price(self):
        ...
        variants = self.variants.filter(is_active=True)
        if variants.exists():
            return max(variant.display_price for variant in variants)
        return 0
