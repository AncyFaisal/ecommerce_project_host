...
class ProductOffer(BaseOffer):
    ...
    products = models.ManyToManyField('products.Product', related_name='product_offers', blank=True)
...
class CategoryOffer(BaseOffer):
    """Offer for one or more categories"""
    categories = models.ManyToManyField('products.Category', related_name='category_offers', blank=False)
    ...
    def __str__(self):
        ...
        if count == 1:
            return f"{self.name} - {self.categories.first().name}"
        elif count > 1:
            return f"{self.name} - {count} categories"
        return self.name
